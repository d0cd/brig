...
| Host services | `.host.brig` virtual domains are an **intentional, scoped relaxation** of this invariant. **Flattened (single-tenant) model:** the cell yaml declares `host_services: [{name, port}]` directly; that's the sole authorization. Warden looks up the port from the cell's own per-cell policy (no separate global registry). Cells with no host_services in yaml have no host-service access. Unknown `.host.brig` domains are blocked. Untrusted profile rejects host_services at parse time. See `_handle_host_service()` in enforce.py. |
