...
- **The untrusted-profile ingress `auth: none` gate reads the trust profile from a trusted source (invariant 4).** The `brig cell start` replay gate read the profile out of `cell-metadata.json`, which invariant 4 names *untrusted* — so the very tampering the gate exists to police (hand-editing that file to add an unauthenticated route to an untrusted cell) could also disarm the gate by dropping the `profile` key. The profile is now stamped as a reserved `brig.profile` podman **container label** at create and read back via `podman inspect`; the schema no longer publishes `profile` at all (v4). Five separate paths that could drop or shadow the marker on the way to the container are closed structurally at the single reconciler choke point rather than individually: `apply_profile`'s dict-only label merge silently discarded profile labels for the list-shaped `labels` the CLI/SDK actually seed (so **no real untrusted cell carried the marker**); a cell-yaml `labels:` block clobbered the profile-merged labels; and a user `--label brig.profile=trusted` could shadow it. **Cells created before this change carry no label** and are not covered by the replay gate — re-create them to get it. Remaining state-dir-read gaps are tracked in `docs/ROADMAP.md` → "Trusted-source hardening".
- **A copied-out symlink-to-directory is refused instead of followed.** `brig cell cp --sanitize` tested `is_dir()` before `is_symlink()`, and `is_dir()` follows the link — so a cell that planted a symlink to a host directory got `_sanitize_tree` and a recursive `xattr` run against the link *target*: a confused-deputy chmod/unlink/xattr on an arbitrary host tree. Symlinks are now rejected before any type test, and `_apply_quarantine` recurses only into a real (non-symlink) directory. Quarantine-xattr failures are now reported rather than swallowed, so an unmarked copied-out file doesn't look Gatekeeper-protected.
- `brig policy show/test/set/rm` validate the cell name against `CELL_NAME_PATTERN` before it reaches a per-cell policy file path; `profile:` must be a bare name (no `/`, `..`, or NUL) in both the cell-yaml validator and `load_profile` itself, which the CLI/SDK reach ahead of validation.
- **A `methods: []` policy rule is no longer enforced asymmetrically.** `matches_method` treated `None` as unrestricted but `[]` as "matches nothing", so an empty-methods *deny* was a no-op on plain HTTP while still blocking the HTTPS CONNECT. Empty now means unrestricted everywhere, matching the existing `paths: []` handling.
- Warden addons harden against tampered inputs (invariant 4): a non-dict or structurally-invalid per-cell policy file is skipped rather than escaping `_reload_cell_policies` — which runs inside `request()`/`http_connect()`, where an unhandled hook exception fails **open**; ingress routes with an out-of-range port are dropped; the notifier's cell name is pattern-checked before it reaches a log path, and its numeric config fields are coerced so a string value can't raise on every flow; a non-positive `log_quota.max_size_mb` falls back to 100MB instead of disabling log rotation. Host-side `cell-metadata.json` readers return their empty default on a non-dict payload instead of raising `AttributeError`.
- Reloading warden's rate-limit config now clears existing token buckets, so a tightened limit reaches already-active cells instead of applying only to cells first seen after the edit.
...
- **`restart: always` cells now self-heal, with an explicit stop respected.** Previously a cell was only replayed when its container had vanished entirely, so a *crashed* or SIGKILLed cell (container present, exited) stayed down until a manual `brig cell start` — and there was no way to keep a cell down across a VM restart short of `brig cell rm`. brig now tracks desired state: `brig cell stop`/`kill` records a stop marker and recovery leaves the cell alone; any other down cell is recovered. This is the systemd `Restart=always` / Docker `unless-stopped` distinction between "it failed" and "I stopped it". `brig system down` is harness-level, not per-cell intent, so it records **no** marker and its cells come back on the next `brig system up`.
- **`brig system watchdog` supervises the whole harness, not just warden.** Each interval it recovers down `restart: always` cells, and when warden is unreachable it brings the *entire stack* up via `brig system up` — which restarts the Lima VM if the host slept and dropped it, something the previous bare warden restart could not do. A successful bring-up resets the failure counter, so a transient VM absence can't exhaust `--max-restarts`. The watchdog is exempt from the "VM is not running" preflight (it exists to fix that state). Run it persistently (launchd agent / process-compose) for cells that survive sleep and reboot.

### Changed

- **Cell metadata schema v3 → v4: the trust `profile` field is removed** (see Security). `workspace.quota`, which the v3 writer already emitted but the v3 schema never documented, is now documented. See `docs/reference/cell-metadata.md`.
...
- **`restart: always` cells are restorable again.** `restore_persisted_cells` re-validates `dataclasses.asdict(spec)` before relaunching, so every optional field the cell.yaml omitted arrives as `None` — and both `_v_user` and `_v_workspace_quota` rejected `None`. Any cell not setting *both* keys was silently skipped with an "invalid persisted spec" warning instead of being relaunched, which is nearly every cell. A regression test now asserts the whole round-trip (a default `CellSpec` must re-validate clean), so the next optional field to grow a validator can't reintroduce this one field at a time.
- `rm` is validated as a boolean in cell yaml; a truthy non-bool (e.g. the string `"no"`) would otherwise silently enable `podman run --rm`, auto-deleting the cell on exit.
- A `sha512` or short-hex image reference no longer suppresses the "unpinned and unverified" warning — OCI manifest digests are sha256, so such a ref was never a usable pin. Uppercase-hex sha256 pins (which the validator accepts) correctly stay silent.
- Dead code removed: `collector.otlp_endpoint_for_warden` / `prometheus_url_for_host` had no callers (both endpoints are built inline at their use sites).
