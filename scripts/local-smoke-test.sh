...
if limactl shell brig -- echo "hello from VM" >/dev/null 2>&1; then
...
if limactl shell brig -- podman --version >/dev/null 2>&1; then
    PODMAN_VER=$(limactl shell brig -- podman --version 2>/dev/null)
...
# Test gVisor.
if limactl shell brig -- test -x /usr/local/bin/runsc; then
    pass "gVisor (runsc) installed in VM"
else
    fail "gVisor not found in VM"
fi
...
WARDEN_STATUS=$(limactl shell brig -- podman inspect warden --format '{{.State.Status}}' 2>/dev/null || echo "not found")
...
RUNTIME=$(limactl shell brig -- podman inspect "brig-$CELL_NAME" --format '{{.HostConfig.Runtime}}' 2>/dev/null)
...
NETWORKS=$(limactl shell brig -- podman inspect "brig-$CELL_NAME" --format '{{range $k, $v := .NetworkSettings.Networks}}{{$k}} {{end}}' 2>/dev/null)
...
HTTP_PROXY=$(limactl shell brig -- podman inspect "brig-$CELL_NAME" --format '{{range .Config.Env}}{{println .}}{{end}}' 2>/dev/null | grep "^http_proxy=" || echo "")
